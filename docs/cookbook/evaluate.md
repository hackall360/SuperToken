...
At minimum `vocab.json` (BPE) or `unigram.vocab` (SentencePiece unigram) must be present; merge metadata enriches compression statistics and `tokenizer.json` records extra provenance in the report.
